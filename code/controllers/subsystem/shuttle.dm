...
/datum/controller/subsystem/shuttle/proc/generate_transit_dock(obj/docking_port/mobile/M)
	...
	if(transit_width > 40 || transit_height > 56)
	...
	var/transit_x = bottomleft.x + (transit_height/2) + abs(union_coords[1])
	var/transit_y = bottomleft.y + (transit_width/2) + abs(union_coords[2])
